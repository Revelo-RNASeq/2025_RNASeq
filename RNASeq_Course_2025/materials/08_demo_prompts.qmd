...
1. Esegui dds <- DESeq(dds)   # ~1-2 minuti
2. Salva subito: saveRDS(dds, "results/dds_deseq.rds")  # DESeq() è lento!
3. Mostra resultsNames(dds) per vedere i coefficienti disponibili
4. Estrai i risultati per il confronto dex trt vs untrt:
   res <- results(dds, name="dex_trt_vs_untrt")
   saveRDS(res, "results/res.rds")
5. Mostra summary(res) con alpha=0.05
6. Ordina per p-value: resOrdered <- res[order(res$pvalue), ]
7. Mostra i top 10 geni
...
1. Verifica il coefficiente: resultsNames(dds) 
   → usa "dex_trt_vs_untrt"
2. Applica lfcShrink:
   resLFC <- lfcShrink(dds, coef="dex_trt_vs_untrt", type="apeglm")
   saveRDS(resLFC, "results/resLFC.rds")
3. Confronta visivamente res vs resLFC con due MA plot affiancati:
   par(mfrow=c(1,2))
   plotMA(res, ylim=c(-2,2), main="MLE (no shrinkage)")
   plotMA(resLFC, ylim=c(-2,2), main="apeglm shrinkage")
4. Salva come 'results/MA_comparison.pdf'
...
1. Dispersion plot: plotDispEsts(dds)
   Salva come 'results/dispersion_plot.pdf'

2. P-value histogram:
   hist(res$pvalue[res$baseMean > 1], breaks=40,
        xlab="p-value", main="P-value distribution")
   Salva come 'results/pvalue_hist.pdf'
...
```
Dai risultati DESeq2 (res e resLFC) del dataset airway:

1. Filtra geni significativi: padj < 0.05 E |log2FoldChange| > 1
   (usa i valori MLE da 'res' per padj, e i valori shrinkati da 'resLFC' per LFC)
2. Separa in UP-regulated (LFC > 1) e DOWN-regulated (LFC < -1)
3. Crea una tabella summary:
   - Totale geni testati (con padj non NA)
   - Significativi con entrambi i criteri
   - UP e DOWN separati
4. Ordina per padj e salva in 'results/DE_genes_airway.csv'
5. Mostra i top 5 geni UP e DOWN con: nome gene, baseMean, LFC shrinkato, padj
